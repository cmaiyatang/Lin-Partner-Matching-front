...
const search = async () => {
  status.value.friends = (await myAxios.post("/friend/searchFriends", {
    friendName: status.value.searchText
  })).data
}
...
onMounted(async () => {
  ...
  status.value.friends = (await myAxios.post("/friend/searchFriends",{})).data
})
...
//   event.CurrentUserTarget.style.backgroundColor = '#e6e6e6';
// }
//
// const resetColor = (event) => {
//   event.CurrentUserTarget.style.backgroundColor = '#fff';
// }
</script>
        ...
        <li v-else v-for="friend in status.friends" :key="friend.id"
          ...
          <div class="avatar-container">
            <img @click="showUser(friend.id)"
                 :src="friend.avatarUrl??defaultPicture" class="avatar" alt="展示好友">
          </div>
...
<!--              <van-icon  max="99" :size="28" name="chat-o"/>-->
